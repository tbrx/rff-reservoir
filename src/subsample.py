...
class Subsampler(object):
    ...
    def __init__(self, points, phi, initial_log_normalizer=None, logging=True, ground_truth_phi=None, test_fns=None):
        ...
        self._compute_phi = phi
        self._each_phi = phi(points)
        self._full_mean_phi = self._each_phi.mean(0)
        self._subset_mean_phi = self._each_phi.mean(0)
        self.M = len(points)
        self.N = len(points)
    ...
    def consider(self, candidate, log_weight=0.0):
        ...
        self._full_mean_phi[:] = weight_next*next_phi + weight_prev*self._full_mean_phi
        ...
        if accepted:
            self._subset_mean_phi += (next_phi - self._each_phi[best_ix])/self.M
